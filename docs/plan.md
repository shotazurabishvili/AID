...
| 7 | ~~Model 4 — ANOVA on intervention typology~~ **Closed without estimation** | 1 | [ADR-0007](decisions/0007-oecd-crs-intervention-typology.md) Rejected 2026-05-23: pre-committed lock gate failed on all three criteria (raw agreement 39 %, κ=0.19, unclassified 76 %). Author decision: drop Model 4 rather than escalate to hand-coding. One session: ADR closure + downstream scope revision (see §5.5 of `findings.md`). Phase-7 obligations withdrawn in `obligations.md`. |
