...
2. The purpose-code-to-bucket mapping puts 82.7 % of projects in `budget_support` and the rule-based classifier puts 4.2 % there — the two methods are not measuring the same construct (the comparator was structurally mis-specified for typology, not just empirically weak).

Both could in principle be repaired (Path B: true LLM-on-text classifier; Path C: iterate keyword rules v2; Path A: hand-code). All three either spend substantial resources on an axis whose CRS-extractability is unproven, or break the no-post-hoc-tuning discipline that PAP-0007 was written to enforce. The researcher move is to scope down rather than spend on rescue work, and treat the failed gate as evidence rather than an obstacle.

Concretely: Model 4 is removed from the paper. The empirical headline is Models 1-2-3 (cross-section, within-FE, multilevel) plus Model 5 (counterfactual simulation). The original five-empirical-models design reduces to four; the negative result is documented in the manuscript.

## Consequences

- Model 4 ANOVA does not run. The obligations (Levene's test, Tukey HSD, η², Cohen's d for all pairs) are withdrawn in `the deposited diagnostic register`.
- Model 5 (counterfactual) **cannot use Model-4 ANOVA effect sizes** — "redirect $1B from input-based to outcome-based aid" simulation needs a different effect-size input. Practical substitute: Model 2's within-country β on log(CRS disbursement) translated through the LAYS reporting layer, with the typology-level redirection reframed as a total-volume / lag-structure / sub-sector counterfactual rather than a four-bucket reallocation.  will lock this redesign.
...
- Brief.md (immutable) still specifies Model 4. Methodology.md §3.8 documents the drop as a downstream-of-brief design revision;  is not edited.
