# ADR-0012: Retirement of UIS multiple imputation (amends ADR-0006)

**Status:** Accepted
**Date:** 2026-05-23
**Phase:** 10 — Pass 1 Statistical Validity, Session 01
**Amends:** [ADR-0006](0006-uis-missingness-strategy.md) (Robustness 2 sub-commitment only; ADR-0006's primary lock — drop UIS from primary — is unaffected.)

## Context

ADR-0006 (Accepted 2026-05-18) committed to three sensitivity directions for the UIS-missingness question:
...
- **Robustness 2 (Phase-5 implementation):** Multiple-imputation UIS-augmented spec on the full sample.

The Robustness 2 commitment was deferred to Phase 5; the actual session log trail (Sessions 11–15) shows it was never executed. At Phase 10 Session 01 (this ADR), we audited intent before deciding whether to execute or formally retire.
...
Retire the MI Robustness-2 sub-commitment from ADR-0006. The UIS-missingness sensitivity now rests on two specs only:

- **Primary** (ADR-0006 Option 3): drop UIS, WDI controls only. **Locked.**
- **Robustness 1**: UIS-augmented listwise spec on the listwise-complete subset. **Executed Phase 10 Session 01.** Result: β_ODA = −1.97 (SE 4.40, N = 41 post-singleton-drop, p > 0.05). The ODA coefficient is sign-flipped vs primary but non-significant; the 95% CIs partially overlap (primary [0.32, 21.95]; listwise [−10.59, 6.66]). This is *weak robustness*: the result is not contradicted but is not strongly confirmed either at the listwise sample size. Reported transparently in `findings.md §5.8` and the Pass 1 audit doc.

This amendment does **not** affect ADR-0006's primary lock (drop UIS from primary). It only retires the deferred Robustness-2 MI sub-spec.

## Consequences

- `mice` need not be added to renv.
- ADR-0009's "may be needed for the imputation model specification" hook (line in ADR-0006 consequences) is moot.
- Pass 1 sign-off (Phase 10 audit doc) reports two UIS-missingness robustness specs, not three.
- The Phase-11 manuscript should report the listwise robustness result alongside the primary, with the explicit note that MI was considered and retired per the MCAR gate + no-fabrication principle. The §6 limits paragraph already owns the UIS-missingness story openly per ADR-0006.
...
The timeline rebuts this. ADR-0006 (Accepted 2026-05-18) explicitly hedged MI's status: "where used at all". The MCAR test result (Phase 2 Session 01, before any Model 2 run) was already on disk and already rejected MCAR at p ≪ 10⁻⁶. The grounds for retiring MI were established *before* any Phase 10 result was generated; the retirement decision is documented in this ADR with the rationale on the record. The listwise result is reported transparently in §5.8 with the same hedging the primary headline gets.
...
Because ADR-0006 made an explicit commitment in writing. Silently dropping it would be exactly the methodological-convenience move the project's discipline pattern (ADR-0007 Rejected, etc.) has avoided throughout. Writing this ADR amendment is the housekeeping cost of honoring the original ADR's status as a load-bearing document.
