...
- **Next concrete action:**
  *(**Phase 11 Session 10 — Render + author final-pass read + submission prep**)*. Content is locked as of Session 34 (10,942w main-text, below 11k *World Development* ceiling; zero placeholders except §3.3 positionality; appendices A/B/C populated; all author-review flags dispositioned). Session 10 sub-components: (a) `cd drafts && quarto render aid_without_learning.qmd --to docx` — DOCX-first to dodge LaTeX-install risk in WSL; verify zero `(?)` citation placeholders in render output; (b) if DOCX renders cleanly, `quarto render aid_without_learning.qmd --to pdf` for the PDF artifact; if LaTeX install fails, document and defer PDF to a Session 11 polish; (c) verify `{{< include >}}` table embeds rendered cleanly — if any misfire, hardcode the .md content into the .qmd as fallback (per Session 09 plan risk register); (d) author final-pass read of the rendered DOCX/PDF — surface any framing-level edits before submission; (e) submission prep: §3.3 positionality fill (4 author specifics required — years/roles, regions, donor relationship type, 1-2 reporting-bias examples) + OSF deposit (code + intermediate data + manuscript snapshot) + cover letter to *World Development* highlighting the path-c methodological contribution + Elsevier Editorial Manager submission. **Verification-grep canonical form (post-Session-08b, used clean in Session 09):** for-loop variant only — see Session 33 log. **Estimated sessions remaining: 1-2** (Session 10 = render + author read + submission prep; optional Session 11 if substantive author-edits surface from the final-pass read).
