...
- **Next concrete action:**
  *(**Phase 11 Session 01 — Manuscript drafting (path-c framing, full draft)**)*. Per plan.md row 11; Quarto manuscript ~10k words; matches brief's section structure with the path-c arc: (a) intro frames the *pre-commit-and-test-honestly* methodology as a contribution; (b) §5 results section orders Model 4 drop (§5.5) → Model 5 counterfactual (§5.6) → Compounding AI Penalty (§5.7) → HLO measure-fragility (§5.8) so the discipline arc builds before the within-country positive headline lands as one identified-but-measure-sensitive finding; (c) §6 Discussion frames each methodological discovery as a contribution to the cross-country aid-learning literature (Sandefur 2018; Vivalt 2020; Pritchett; Deaton & Cartwright 2018 as the comparator lineage). Estimated 6-10 sessions for the full draft on this branch. Drafts land under `drafts/*.qmd`; commit per Phase-11 session; merge back to `main` only for shared-infrastructure changes per the Branching Strategy section above.
