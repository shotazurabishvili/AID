...
##### Counterfactual at the high-shock end: a $1B annual redirect

A $1B annual increase distributed across the 173-country Model 2 estimation sample = **$5.78M per country on average**, which is **9.7%** of the median baseline ($59.7M) and **5.49%** of the sample's total annual education aid ($18.2B). This lands in the low-shock band of the headline table; a $1B redirect is *not* well-described by the highest-shock scenarios above. Applying the entire $1B to a single country would push that country 18× above its baseline — outside the data support Model 2 was identified on, and I do not project there.

##### Limits acknowledged

- **Identification:** Model 2 is static FE on small-T (T_eff ≤ 4 HCI cycles per country); GMM is infeasible at this T (PAP-0010). β is identified within-country over time but does not preclude unmeasured time-varying confounding.
- **Composition:** the counterfactual is on aggregate CRS disbursement only. The input-vs-outcome typology question is unanswered (PAP-0007 Rejected; §4.4).
...
- **Implementation quality, political economy, and absorptive capacity** caveats apply throughout.
