...
# and runs the Phase-2 diagnostics that feed ADR-0006 (MCAR on the production
...
message(sprintf("[audit] mode = %s; PANEL_PATH = %s",
                if (is_audit_mode) "AUDIT (Phase 1)" else "PRODUCTION (Phase 2)",
                PANEL_PATH))
...
# In production mode, panel is already filtered to ADR-0002 universe; use
...
if (is_audit_mode) {
  ...
  mcar_result <- run_mcar(mcar_panel, MCAR_COLS, "audit 6-col 2010-2020 ∩ ADR-0002")
